...
public unsafe class EntityViewUpdater : MonoBehaviour {
  ...
  EntityView CreateView(QuantumGame game, Frame f, EntityRef handle, Quantum.EntityView view, EntityViewBindBehaviour createBehaviour) {
    ...
    if (TryGetTransform(f, handle, out Vector3 position, out Quaternion rotation)) {
      instance = CreateEntityViewInstance(asset, view.Guid, position, rotation);
    } else {
  ...
  protected void OnEntityViewInstantiated(QuantumGame game, Frame f, EntityView instance, EntityRef handle) {
  ...
  protected virtual EntityView CreateEntityViewInstance(EntityViewAsset asset, AssetGuid guid = default, Vector3? position = null, Quaternion? rotation = null) {
